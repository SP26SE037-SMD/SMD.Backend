...
public class SubjectController {
        ...
        @GetMapping
        @Operation(summary = "Advanced search subjects with pagination", description = "Search by 'code' or 'name' directly at the database level. Filters by status is also supported.")
        public ResponseObject<PagedResponse<SubjectResponse>> getAll(
                        ...
                        @Parameter(description = "Filter subjects by their current lifecycle status. Valid values are:\n"
                                        +
                                        "| Status | Description |\n" +
                                        "| :--- | :--- |\n" +
                                        ...
                                        +
                                        "| **ARCHIVED** | **Lưu trữ:** Subject is retired from active curricula. Kept as Read-only for historical academic records. |") @RequestParam(required = false) String status,
                ...
                return ResponseObject.<PagedResponse<SubjectResponse>>builder()
                                .data(PagedResponse.of(subjectService.getAll(search, searchBy, status, departmentId,
                                                pageable, userId)))
        ...
        @GetMapping("/{id}")
        @Operation(summary = "Get subject detail by ID", description = "Retrieves full subject details including Department and CLOs using JOIN FETCH for optimal performance.")
        ...
        public ResponseObject<SubjectResponse> getByCode(@PathVariable String subjectCode,
                        @AuthenticationPrincipal Jwt jwt) {
                String userId = jwt.getClaimAsString("accountId");
                return ResponseObject.<SubjectResponse>builder()
                                .status(1000)
        ...
        @Operation(summary = "Publish a subject syllabus", description = "Change subject status from Draft to Published. Requires 'SUBJECT_PUBLISH' authority and a valid decision number.")
        ...
        @PreAuthorize("hasAuthority('SUBJECT_UPDATE')")
        @Operation(summary = "Update Subject Lifecycle Status (Cập nhật trạng thái vòng đời Môn học)", description = "### 📚 Quy trình điều phối Môn học (Subject Workflow):\n"
                        +
        ...
        public ResponseObject<Integer> changeStatus(
                ...
                int updatedCount = subjectService.updateAllSubjectStatusInCurriculum(curriculum_id, department_id,
                                newStatus, oldStatus);
        ...
        @PreAuthorize("hasAuthority('SUBJECT_UPDATE')")
        @Operation(summary = "Update Subject status by Curriculum (optional oldStatus)", description = "Cập nhật trạng thái môn học theo curriculum. Trường hợp này department được coi là null.")
        public ResponseObject<SubjectResponse> changeStatusByCurriculum(
                        @PathVariable UUID curriculum_id,
                        @RequestParam String newStatus,
                        @RequestParam(required = false) String oldStatus) {
                int updatedCount = subjectService.updateAllSubjectStatusInCurriculum(curriculum_id, null, newStatus,
                                oldStatus);
                return ResponseObject.<SubjectResponse>builder()
                                .status(1000)
        ...
        @PreAuthorize("hasAuthority('SUBJECT_CREATE')")
        @Operation(summary = "Import subjects from Excel", description = "Import subject records from Excel columns: subjectCode, subjectName, credits, degreeLevel, timeAllocation, description, departmentCode, mintopass, studentLimit, studentTasks, scoringScale")
        ...
        @Operation(summary = "Get subjects by Department ID", description = "Retrieve a list of subjects belonging to a specific department. \n\n"
                        +
