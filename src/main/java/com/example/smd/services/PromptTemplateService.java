...
public class PromptTemplateService {
    ...
    //Biến gắn vào statics
    private Map<String, String> promptMap;

    @PostConstruct
    public void init() throws Exception {
        var resource = resourceLoader.getResource("classpath:prompt.json");
        this.promptMap = objectMapper.readValue(
                resource.getInputStream(),
                new TypeReference<Map<String, String>>() {}
        );
    }

    // Hàm lấy prompt theo Enum
    public String get(PromptKey key) {
        return promptMap.get(key.name());
    }
