...
import com.example.smd.services.PdfExtractService;
...
//        @PreAuthorize("hasAuthority('CURRICULUM_UPDATE')")
        ...
        // ─────────────────────────────────────────────────────────────────────
        // PDF Subject Extraction APIs
        // ─────────────────────────────────────────────────────────────────────

        /**
         * Trích xuất danh sách môn học từ section "7. Program content" của file PDF
         * và trả về kết quả dưới dạng chuỗi văn bản theo định dạng chuẩn.
         *
         * Định dạng đầu ra:
         * SubjectName (SubjectCode|Semester|Credits|Theory|Practical|SelfStudy), ...
         *
         * Ví dụ:
         * Marxist-Leninist philosophy (001535|1|3|33|24|90), Linear Algebra (001053|1|2|26|8|60)
         */
        @PostMapping(value = "/extract-subjects", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @Operation(
                summary = "Extract subjects from PDF curriculum file (returns formatted string)",
                description = "Reads the uploaded PDF, locates section \"7. Program content\" and extracts" +
                        " the subject table into a formatted string.\n\n" +
                        "**Output format per subject:**\n" +
                        "`SubjectName (SubjectCode|ExpectedSemester|Credits|Theory|Practical|SelfStudy)`\n\n" +
                        "Multiple subjects are separated by `, `.\n\n" +
                        "**Notes:**\n" +
                        "- Processing is done purely with Apache PDFBox (no AI).\n" +
                        "- Rows without a valid Subject Code (4–10 digit numeric string) are skipped.\n" +
                        "- Sub-header rows (e.g. \"General education knowledge\") are automatically filtered out."
        )
        public ResponseObject<String> extractSubjectsAsString(
                @Parameter(description = "PDF file of the training curriculum (chương trình đào tạo)")
                @RequestParam("file") MultipartFile file
        ) {
                try {
                        String result = pdfExtractService.extractSubjectsFromPdf(file);
        ...
        public ResponseObject<String> extractSubjectsAndReference(
                ...
                try {
                        String result = pdfExtractService.extractSubjectsAndReferenceFromPdf(file);
                        return ResponseObject.<String>builder()
                                .status(1000)
                                .data(result)
                                .message("PDF subject extraction completed successfully")
        ...
        /**
         * Trích xuất danh sách môn học từ section "7. Program content" của file PDF
         * và trả về kết quả dưới dạng danh sách DTO (thuận tiện cho việc xử lý phía client).
         */
        @PostMapping(value = "/extract-subjects/list", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @Operation(
                summary = "Extract subjects from PDF curriculum file (returns structured list)",
                description = "Reads the uploaded PDF, locates section \"7. Program content\" and extracts" +
                        " the subject table into a structured list of objects.\n\n" +
                        "Each item contains: `subjectCode`, `subjectName`, `expectedSemester`," +
                        " `numberOfCredits`, `theory`, `practical`, `selfStudy`.\n\n" +
                        "- Processing is done purely with Apache PDFBox (no AI).\n" +
                        "- Rows without a valid Subject Code are skipped."
        )
        public ResponseObject<List<PdfSubjectExtractDTO>> extractSubjectsAsList(
                @Parameter(description = "PDF file of the training curriculum (chương trình đào tạo)")
                @RequestParam("file") MultipartFile file
        ) {
                try {
                        List<PdfSubjectExtractDTO> result = pdfExtractService.extractSubjectDTOs(file);
                        return ResponseObject.<List<PdfSubjectExtractDTO>>builder()
                                .status(1000)
                                .data(result)
                                .message("PDF subject extraction completed. Found " + result.size() + " subjects.")
                                .build();
                } catch (Exception e) {
                        return ResponseObject.<List<PdfSubjectExtractDTO>>builder()
                                .status(9999)
        ...
        // ─────────────────────────────────────────────────────────────────────
        // PDF Reference Book Extraction APIs
        // ─────────────────────────────────────────────────────────────────────

        /**
         * Trích xuất danh sách tài liệu tham khảo từ section "*List of Reference Books" của file PDF
         * và trả về kết quả dưới dạng chuỗi văn bản định dạng chuẩn.
         *
         * Định dạng đầu ra:
         * ReferenceName (ReferenceCode|SubjectCode|AuthorName|Publisher|PublishedYear), ...
         */
        @PostMapping(value = "/extract-references", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @Operation(
                summary = "Extract reference books from PDF curriculum file (returns formatted string)",
                description = "Reads the uploaded PDF, locates the section \"*List of Reference Books\"" +
                        " and extracts the reference book table into a formatted string.\n\n" +
                        "**Output format per reference:**\n" +
                        "`ReferenceName (ReferenceCode|SubjectCode|AuthorName|Publisher|PublishedYear)`\n\n" +
                        "Multiple references are separated by `, `.\n\n" +
                        "**Notes:**\n" +
                        "- Processing is done purely with Apache PDFBox (no AI).\n" +
                        "- Subject Code may contain multiple codes separated by commas (e.g. `001535,001536`).\n" +
                        "- Index, Edition and Subject Name columns are ignored in output."
        )
        public ResponseObject<String> extractReferencesAsString(
                @Parameter(description = "PDF file of the training curriculum (chương trình đào tạo)")
                @RequestParam("file") MultipartFile file
        ) {
                try {
                        String result = pdfExtractService.extractReferenceBooksFormatted(file);
                        return ResponseObject.<String>builder()
                                .status(1000)
                                .data(result)
                                .message("PDF reference book extraction completed successfully")
                                .build();
                } catch (Exception e) {
                        return ResponseObject.<String>builder()
                                .status(9999)
                                .message("Failed to extract references from PDF: " + e.getMessage())
                                .build();
                }
        }
         ...
         * và trả về kết quả dưới dạng danh sách DTO (thuận tiện cho việc xử lý phía client).
         */
        @PostMapping(value = "/extract-references/list", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @Operation(
                summary = "Extract reference books from PDF curriculum file (returns structured list)",
                description = "Reads the uploaded PDF, locates the section \"*List of Reference Books\"" +
                        " and extracts the reference book table into a structured list of objects.\n\n" +
                        "Each item contains: `referenceCode`, `referenceName`, `authorName`," +
                        " `publisher`, `publishedYear`, `subjectCode`.\n\n" +
                        "- Processing is done purely with Apache PDFBox (no AI).\n" +
                        "- Multi-line reference names, authors, and publishers are merged automatically.\n" +
                        "- Multiple subject codes per reference are normalized to comma-separated format."
        )
        public ResponseObject<List<ReferenceBookExtractDTO>> extractReferencesAsList(
                @Parameter(description = "PDF file of the training curriculum (chương trình đào tạo)")
                @RequestParam("file") MultipartFile file
        ) {
                try {
                        List<ReferenceBookExtractDTO> result = pdfExtractService.extractReferenceBooks(file);
                        return ResponseObject.<List<ReferenceBookExtractDTO>>builder()
                                .status(1000)
                                .data(result)
                                .message("PDF reference book extraction completed. Found " + result.size() + " references.")
                                .build();
                } catch (Exception e) {
                        return ResponseObject.<List<ReferenceBookExtractDTO>>builder()
                                .status(9999)
